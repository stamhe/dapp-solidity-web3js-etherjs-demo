...
contract MultisigWallet {
    ...
    function execTransaction(
        ...
        // 编码交易数据，计算哈希
        bytes32 txHash = encodeTransactionData(to, value, nonce, block.chainid);
        nonce++; // 增加nonce
    ...
    function encodeTransactionData(
        ...
        bytes32 safeTxHash = keccak256(abi.encode(to, value, _nonce, chainid));
        return safeTxHash;
    }

    function execTokenTransfer(
        address to_,
        uint256 amount_,
        bytes memory signatures_
    ) public payable virtual returns (uint8) {
        // 编码交易数据，计算哈希
        bytes32 txHash = encodeTokenTransfer(
            to_,
            amount_,
            nonce,
            block.chainid
        ...
        nonce++; // 增加nonce
        checkSignatures(txHash, signatures_); // 检查签名
        bool success = token.transfer(to_, amount_);
        require(success, "WTF5004");
        if (success) {
    ...
    function encodeTokenTransfer(
        ...
        // erc20 token.transfer => function transfer(address, uint256)
        // bytes memory newCalldata = abi.encodeWithSignature("transfer(address, uint256)",_to,amount_);

        bytes32 safeTxHash = keccak256(
            abi.encode(to_, amount_, nonce_, chainid_)
        );
        return safeTxHash;
    }
}
