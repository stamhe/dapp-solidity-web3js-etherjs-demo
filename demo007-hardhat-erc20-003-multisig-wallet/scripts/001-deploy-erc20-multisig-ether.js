...
async function main() {
    ...
    const MultisigWalletDeploy = await MultisigWalletFactory.deploy(
        ...
        2,
        {
            value: ethers.utils.parseEther("1.11"), // 同时给多签合约转账 1.11 ether
        }
    );
    ...
    console.log(
        `MultisigWallet addr = ${MultisigWalletAddr}, lockAmount = ${balance0} ${tokenSymbol}, ${etherBalance0} ether`
    );

    // 4. 构造交易数据 - token
    const _to = "0xA1c3e8cC2faCda678C861101bE1232c87625392f";
    const _amount = ethers.utils.parseEther("1.11");
    const tx1Hash = await MultisigWallet.encodeTokenTransfer(_to, _amount, nonce, chainid);
    ...
    console.log(
        `MultisigWallet addr = ${MultisigWalletAddr}, lockAmount = ${balance2} ${tokenSymbol}, ${etherBalance2} ether`
    );
    ...
    nonce++;
    const _toEther = "0xA1c3e8cC2faCda678C861101bE1232c87625392f";
    const _amountEther = ethers.utils.parseEther("0.1");
    const _dataEther = "0x0";
    const tx1HashEther = await MultisigWallet.encodeTransactionData(
        _toEther,
        _amountEther,
        nonce,
        chainid
    ...
    console.log("tx1HashEther: ", tx1HashEther);

    const toBalance10Tmp = await provider.getBalance(_toEther);
    const toBalance10 = ethers.utils.formatEther(toBalance10Tmp);

    const etherBalance10Tmp = await provider.getBalance(MultisigWalletAddr);
    const etherBalance10 = ethers.utils.formatEther(etherBalance10Tmp);

    console.log(
        `MultisigWallet ${MultisigWalletAddr} ${etherBalance10} ether, toAddr ${_toEther} ${toBalance10}`
    );

    // 01. 对交易数据进行多签签名
    // This array representation is 32 bytes long
    const tx1BytesEther = ethers.utils.arrayify(tx1HashEther);
    // To sign a hash, you most often want to sign the bytes
    // const tx1Signature = await hre.ethers.provider.getSigner(0).signMessage(tx1Bytes);
    const tx1Signature0Ether = await hre.ethers.provider.getSigner(0).signMessage(tx1BytesEther);
    const tx1Signature1Ether = await hre.ethers.provider.getSigner(1).signMessage(tx1BytesEther);
    console.log(`tx1-0: ${tx1Signature0Ether}, tx1-1: ${tx1Signature1Ether}`);
    // 组装多签签名数据
    const tx1SignatureEther = tx1Signature0Ether.concat(tx1Signature1Ether.substring(2));
    console.log(`tx1SignatureEther: ${tx1SignatureEther}`);

    // 02. 发起交易
    const tx1ResEther = await MultisigWallet.execTransaction(
        _toEther,
        _amountEther,
        tx1SignatureEther
    );
    ...
    // 03. 转账以后，等一个区块时间再执行后面的逻辑
    await new Promise(r => setTimeout(r, 5 * 1000));

    const toBalance13Tmp = await provider.getBalance(_toEther);
    const toBalance13 = ethers.utils.formatEther(toBalance13Tmp);

    const etherBalance13Tmp = await provider.getBalance(MultisigWalletAddr);
    const etherBalance13 = ethers.utils.formatEther(etherBalance13Tmp);

    console.log(
        `MultisigWallet ${MultisigWalletAddr} ${etherBalance13} ether, toAddr ${_toEther} ${toBalance13}`
    );
}
